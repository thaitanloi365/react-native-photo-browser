...
export default class Photo extends Component {
  ...
  _renderProgressIndicator() {
    ...
    if (progress < 1) {
      ...
      return <ProgressElement progress={progress} thickness={20} color={"white"} />;
    }
    return null;
  }
  ...
  _renderSelectionButton() {
    ...
    if (thumbnail) {
      ...
      buttonImage = <Image source={icon} style={styles.thumbnailSelectionIcon} />;
    } else {
      ...
      buttonImage = <Image style={styles.fullScreenSelectionIcon} source={icon} />;
    }

    return <TouchableWithoutFeedback onPress={this._toggleSelection}>{buttonImage}</TouchableWithoutFeedback>;
  }
