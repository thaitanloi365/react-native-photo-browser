...
export default class FullScreenContainer extends React.Component {
  ...
  _renderItem({ item, index }) {
    const { displaySelectionButtons, onMediaSelection, useCircleProgress, photoWidth, photoHeight } = this.props;
  ...
  render() {
    const { displayNavArrows, alwaysDisplayStatusBar, displayActionButton, onGridButtonTap, enableGrid } = this.props;
    const { controlsDisplayed, currentMedia } = this.state;
    const BottomBarComponent = this.props.bottomBarComponent || BottomBar;
    const height = Constants.TOOLBAR_HEIGHT + 16 + getBottomSpace();
    return (
      <View style={styles.flex}>
        <StatusBar
